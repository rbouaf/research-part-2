...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, mode):
        ...
        if not MyDataset.all_imgs_and_labels:
            MyDataset.all_imgs_to_annotations = pickle.load(open('doc2vec_features.pickle', 'rb')) # get all image-annotations
            MyDataset.add_dim = list(MyDataset.all_imgs_to_annotations.values())[0].size 
            paths = pickle.load(open('train_test_paths.pickle', 'rb'))['train_imgs'] # get all paths
            for path in paths:
                if '/left/' in path:
...
def main():
    ...
    for e in tqdm(range(1, 50), ascii=True, desc='Epoch'):
        ...
        try:
            torch.save(img_model.module.state_dict(), 'classifiers2/img_model_{}.pth'.format(e))
        except:
            print('Failed saving')
            continue
